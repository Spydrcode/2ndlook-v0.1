...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      await disconnectConnection(installationId, "square");
    } catch (error) {
      console.error("Square reconnect token clear error:", error);
    }

    return NextResponse.redirect(new URL("/api/oauth/square/start", appUrl));
  } catch (error) {
    console.error("Square reconnect error:", error);
