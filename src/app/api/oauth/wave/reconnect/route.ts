...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      await disconnectConnection(installationId, "wave");
    } catch (error) {
      console.error("Wave reconnect token clear error:", error);
    }

    return NextResponse.redirect(new URL("/api/oauth/wave/start", appUrl));
  } catch (error) {
    console.error("Wave reconnect error:", error);
