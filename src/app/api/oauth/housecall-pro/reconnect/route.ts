...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      await disconnectConnection(installationId, "housecall-pro");
    } catch (error) {
      console.error("Housecall Pro reconnect token clear error:", error);
    }

    return NextResponse.redirect(new URL("/api/oauth/housecall-pro/start", appUrl));
  } catch (error) {
    console.error("Housecall Pro reconnect error:", error);
