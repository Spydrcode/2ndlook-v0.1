...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      await disconnectConnection(installationId, "quickbooks");
    } catch (error) {
      console.error("QuickBooks reconnect token clear error:", error);
    }

    return NextResponse.redirect(new URL("/api/oauth/quickbooks/start", appUrl));
  } catch (error) {
    console.error("QuickBooks reconnect error:", error);
